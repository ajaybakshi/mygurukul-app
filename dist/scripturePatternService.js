...
class ScripturePatternService {
    ...
    initialize() {
        ...
        const missingScriptures = expectedScriptures.filter(scripture => !exports.SCRIPTURE_PATTERNS[scripture]);
     ...
     */
    extractVerseText(line, scriptureFile) {
        if (!this.initialized) {
            this.initialize();
        }
        const config = exports.SCRIPTURE_PATTERNS[scriptureFile];
        if (!config) {
     ...
     */
    getScriptureConfig(scriptureFile) {
        if (!this.initialized) {
            this.initialize();
        }
        return exports.SCRIPTURE_PATTERNS[scriptureFile] || null;
    }
    /**
     * Get all scripture files
     */
    getAllScriptureFiles() {
        return Object.keys(exports.SCRIPTURE_PATTERNS);
    }
    /**
     * Get extractor type for scripture
     */
    getExtractorType(scriptureFile) {
        const config = this.getScriptureConfig(scriptureFile);
        return config ? config.extractor : null;
    }
}
exports.ScripturePatternService = ScripturePatternService;
